...
const addTransaction = async (request, h) => {
  try {
    const { id, amount, category, date, notes } = request.payload;
...
const updateIncomeTransaction = async (request, h) => {
  try {
    ...
    if (!transaction) {
      return h
        .response({ error: "Transaksi income tidak ditemukan" })
        .code(404);
    }
    ...
    return h
      .response({
        message: "Transaksi income berhasil diperbarui",
        transaction,
      })
      .code(200);
  } catch (error) {
...
const updateExpenseTransaction = async (request, h) => {
  try {
    ...
    if (!transaction) {
      return h
        .response({ error: "Transaksi expense tidak ditemukan" })
        .code(404);
    }
    ...
    return h
      .response({
        message: "Transaksi expense berhasil diperbarui",
        transaction,
      })
      .code(200);
  } catch (error) {
